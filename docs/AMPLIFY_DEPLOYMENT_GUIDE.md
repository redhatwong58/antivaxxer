...
This guide covers deploying ANTIVAXXER to AWS using Amplify (frontend) + App Runner (API) with all supporting services. Follow it in order — each phase has dependencies on the previous.

For the alternative Vercel-based deployment, see `DEPLOYMENT_GUIDE.md`.

> 🚀 **First production launch?** This guide handles the AWS infrastructure setup
> (Amplify, App Runner, RDS, Secrets Manager). For the operator setup tasks that
> happen ONCE before the very first launch — SES production access, first admin
> user creation, third-party webhook configuration, final smoke test — see
> **`PRE_LAUNCH_CHECKLIST.md`**.

> **v5.4.6 callout (Cloudflare Turnstile wired):** Add `TURNSTILE_SECRET_KEY` to App Runner env (or `antivaxxer/prod/turnstile-secret` in Secrets Manager). Add `NEXT_PUBLIC_TURNSTILE_SITE_KEY` to Amplify env. Both optional — system gracefully degrades when unset.

> **v5.4.0 callout (Shippo):** Requires Prisma migration (5 new columns on `orders`). New env vars: `SHIPPO_API_KEY`, `SHIPPO_FROM_*` (sender address). Configure tracking webhook in Shippo dashboard pointing to `https://api.antivaxxer.com/api/webhooks/shippo`. Optional — site works without Shippo (manual label flow). Add `antivaxxer/prod/shippo-api-key` to Secrets Manager (Phase 5) and map it to `SHIPPO_API_KEY` in App Runner env.

> **v5.3.9 callout (CRITICAL FIXES):** Requires Prisma migration (`failed_webhooks` table). Webhook handler is now atomic with row locks.

> **v5.3.8 callout:** Pure code deploy — no new env vars, no Prisma migration. Adds Stripe refund button, per-order ops fulfillment email, and Stripe Tax activation flag.

> **v5.3.7 callout:** Pure code deploy — no new env vars, no Prisma migration. Adds order line-item editing, product status `coming_soon`/`prelaunch`, and an automated `pending → processing` transition on Stripe payment success.

> **v5.3.6 callout:** Pure code deploy — no new env vars, no Prisma migration. After deploying, smoke-test the four new admin pages (Dashboard, Inventory, Promos, Customers).

> **v5.3.5 callout** (read this if upgrading from a pre-v5.3.5 deploy):
> 1. Phase 6 (database migrations) now includes a new migration `20260414000000_add_password_reset` that adds two columns to `users`. `npx prisma migrate deploy` picks this up automatically.
> 2. Phase 5 (Secrets Manager) — verify `NEXT_PUBLIC_SITE_URL` is in the API server's environment, not just the frontend's. The password reset email needs it to build the link.
> 3. Phase 4 (SES) — sandbox mode silently drops password reset emails to unverified addresses. Request production access **before launch** or new users can't reset their passwords.
> 4. Phase 12 (admin user) — the frontend admin layout is now hard-gated. Skipping the admin user step locks everyone out of `/admin` (the redirect goes to `/account/login` then `/403`).
...
| `antivaxxer/prod/jwt-secret` | from `openssl rand -base64 32` (can be the same as nextauth-secret if you want — both verify the same JWTs) |
...
| `antivaxxer/prod/site-url` | `https://antivaxxer.com` — **required by API for password reset emails (v5.3.5)**, mapped to `NEXT_PUBLIC_SITE_URL` env var |
...
5. Visit `/admin` — you should see the admin UI

> **v5.3.5:** if you skip this step or get the email wrong, **nobody can access `/admin`**. The frontend layout is server-gated and redirects all non-admin sessions to `/403`. Don't deploy to production without doing this first.

### 12.2 Smoke test the admin gate (v5.3.5)

| Action | Expected |
|---|---|
| Open an incognito window, visit `/admin` | Redirects to `/account/login?callbackUrl=/admin` |
| Sign in as a regular customer (no admin role), visit `/admin` | Redirects to `/403` showing "ACCESS DENIED" |
| Sign in as the admin user from 12.1, visit `/admin` | Admin UI loads with the nav bar showing your email |

If any of these don't behave as listed, check that `NEXTAUTH_SECRET` and `JWT_SECRET` are set in **both** the frontend (Amplify env vars) and the API (Secrets Manager) — they need to be the same value or the JWTs won't verify.

### 12.3 Smoke test password reset (v5.3.5)

| Action | Expected |
|---|---|
| Visit `/account/login`, click "Forgot password?" | Lands on `/account/forgot-password` |
| Submit your real email | "CHECK YOUR EMAIL" success page |
| Submit a fake email | Same "CHECK YOUR EMAIL" page (anti-enumeration) |
| Inbox check | Reset email arrives, branded, with a link |
| Click the link | Lands on `/account/reset-password/[token]` |
| Pick a new password (8+ chars), confirm | Success → redirects to login after 2.5s |
| Sign in with the new password | Works |
| Wait 1 hour, click an old reset link | "This reset link has expired" with "Request a new link" CTA |

If the email never arrives:
- Check SES is in **production** mode (`aws ses get-account-sending-enabled`)
- Check `SES_FROM_EMAIL` is verified in SES
- Check API server logs for `[EMAIL]` lines — failures are logged but don't block the response
- Check `NEXT_PUBLIC_SITE_URL` is set on the **API** server (not just the frontend) — without it the link defaults to localhost and is unusable

### 12.4 Smoke test the full purchase flow
